...
public class ComponentDeMuxGenerator : IIncrementalGenerator
{
    ...
    // try { foreach (ISymbol? classSymbol in classes.Distinct(SymbolEqualityComparer.Default)) {
    // foreach (SyntaxTree syntaxTree in compilation.SyntaxTrees) { SemanticModel semanticModel =
    // compilation.GetSemanticModel(syntaxTree); IEnumerable<ClassDeclarationSyntax>
    // classDeclarations = syntaxTree.GetRoot().DescendantNodes() .OfType<ClassDeclarationSyntax>()
    // .Where(classSyntax => classSyntax.AttributeLists.Any(attributeList =>
    // attributeList.Attributes.Any(attribute => IsComponentDeMuxAttribute(attribute))));
