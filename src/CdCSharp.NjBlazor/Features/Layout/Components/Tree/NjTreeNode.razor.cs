...
public partial class NjTreeNode : NjComponentBase
{
    ...
    /// </summary>
    /// <value>The title to be rendered as a fragment.</value>
    [Parameter]
    ...
    /// <summary>Gets or sets an optional function to open the node.</summary>
    /// <value>The function to open the node.</value>
    [Parameter]
    public Func<NjTreeNode, bool>? OpenFunction { get; set; }

    /// <summary>Gets or sets the parent NjTree in the cascading parameter.</summary>
    /// <value>The parent NjTree.</value>
    [CascadingParameter]
    public NjTree ParentTree { get; set; } = default!;

    /// <summary>Gets or sets the parent node in a cascading parameter.</summary>
    /// <value>The parent node in a cascading parameter.</value>
    [CascadingParameter]
    public NjTreeNode? ParentNode { get; set; }

    /// <summary>Gets or sets the list of child nodes.</summary>
    /// <value>The list of child nodes.</value>
    public List<NjTreeNode> ChildNodes { get; set; } = [];

    /// <summary>Represents the state of being open.</summary>
    ...
    /// <summary>
    /// This method is called when the parameters are set for the current node.
    /// It adds the current node as a child to the parent tree if the parent node is null,
    /// otherwise, it adds the current node as a child to the parent node.
    /// </summary>
    /// <remarks>
    /// If the parent node is null, the current node is added to the parent tree.
    /// If the parent node is not null, the current node is added to the parent node.
    /// </remarks>
    ...
    /// After adding the node, the state of the node is updated, and the parent tree is notified of the change.
    /// </remarks>
